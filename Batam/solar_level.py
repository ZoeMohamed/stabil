...
class Solar():
    ...
    def __init__(self):
        ...
        self.topic = "batam/Solar-level/SENSOR"
        self.status = "batam/Solar-level/LWT"
        self.tool_status = ""
        self.table_name = "solarlevel_batams"
        self.client_id = f'python-mqtt-solarlevel_batams{random.randint(0, 1000)}'
            ...
            while self.Messagereceived != True:
                ...
                while True:
                    ...
                    if self.lowest_volt is not None:
                        ...
                        self.mydb.execute(f"INSERT INTO {self.table_name} (topic,message,unit,date,created_at) VALUES (%s,%s,%s,%s,%s)", (
                            self.topics, str(self.full_message), self.lowest_volt, datetime.date.strftime(
                                datetime.datetime.now(), "%m/%d/%Y/%H:%M:%S"), datetime.datetime.now()))
                        self.db.commit()

                        self.lowest_volt = None

                    elif(sekarang - time_stamp) >= self.time_trigger and len(self.arr_normal_unit) != 0:
                        print("lebih dari 20 Mins")

                        self.mydb.execute(f"INSERT INTO {self.table_name} (topic,message,unit,date,created_at) VALUES (%s,%s,%s,%s,%s)", (
                            self.arr_normal_topic[-1], str(self.arr_normal_message[-1]), self.arr_normal_unit[-1], datetime.date.strftime(
                                datetime.datetime.now(), "%m/%d/%Y/%H:%M:%S"), datetime.datetime.now()))
                        self.db.commit()
                        self.arr_normal_message = []
                        self.arr_normal_topic = []
    ...
    def on_message(self, client, userdata, message):
        ...
        if(message.payload.decode('utf-8') == "Online" or message.payload.decode('utf-8') == "Offline"):
            ...
            unit = int(convertedDict['ANALOG']['A0'])
            ...
            if(len(self.comp_arr) == 2):
                if(abs(self.real_time_volt - self.last_volt)) >= self.voltage_indicator:
                    ...
                    self.arr_normal_unit.append(unit)
                    self.arr_normal_message.append(convertedDict)
                    self.arr_normal_topic.append(topic)

                    if(len(self.arr_normal_unit) == 2 and len(self.arr_normal_message) == 2 and len(self.arr_normal_topic) == 2):
                        self.arr_normal_unit.pop(0)
                        self.arr_normal_message.pop(0)
                        self.arr_normal_topic.pop(0)

                    print("KALO LEN NYA 2 LISTRIK DAH STABIL")

                    print(self.arr_normal_unit)
